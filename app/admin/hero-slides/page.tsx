  ...
  return (
    <div className="p-8">
      ...
      <div className="grid gap-6">
        {slides.map((slide) => {
          const isFirstSlide = slide.display_order === 0;

          return (
            <div
              key={slide.id}
              className={`bg-white border rounded-lg p-6 flex gap-6 ${isFirstSlide ? 'border-yellow-400 bg-yellow-50' : 'border-gray-200'}`}
            >
              {/* Image/Video Preview */}
              <div className="w-48 h-32 relative rounded-lg overflow-hidden bg-gray-100 flex-shrink-0">
                {isFirstSlide ? (
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-br from-gray-900 to-gray-700 text-white">
                    <svg className="w-12 h-12 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" />
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <span className="text-xs font-bold text-center px-2">hero.mp4</span>
                    <span className="text-xs text-gray-300 mt-1">(Video)</span>
                  </div>
                ) : (
                  <Image
                    src={slide.image_url}
                ...
                <div>
                  ...
                  {isFirstSlide && (
                    <p className="text-xs text-yellow-700 mt-2 font-medium">
                      ⚠️ This slide displays the <code>hero.mp4</code> video file instead of an image
                    </p>
                  )}
                </div>
                <div className="flex items-center gap-2">
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            {/* Warning for first slide */}
            {editingSlide && editingSlide.display_order === 0 && (
              <div className="mx-6 mt-4 p-4 bg-yellow-50 border border-yellow-400 rounded-lg">
                <div className="flex items-start gap-3">
                  <svg className="w-5 h-5 text-yellow-600 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                  </svg>
                  <div>
                    <h4 className="text-sm font-bold text-yellow-800">First Slide Uses Video</h4>
                    <p className="text-xs text-yellow-700 mt-1">
                      This slide (Display Order: 0) displays the <code className="bg-yellow-100 px-1 rounded">hero.mp4</code> video file on the homepage. The image URL field below is only used for fallback purposes if the video fails to load.
                    </p>
                  </div>
                </div>
              </div>
