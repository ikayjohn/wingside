-- Simple Referral System Deployment Script
-- This version avoids complex loops for better compatibility
...
BEGIN
    ...
    IF table_count = 4 THEN
        ...
        RAISE NOTICE 'Tables created: referrals, referral_rewards, referral_settings, referral_shares';
        RAISE NOTICE 'Functions, triggers, indexes, and RLS policies updated/created';
        RAISE NOTICE 'Active referral settings count: %', settings_count;
    ELSE
        RAISE NOTICE '❌ Deployment issue. Expected 4 tables, found %', table_count;
    END IF;
END $$;

-- 9. Done! Run these verification queries separately if needed:

-- To check tables:
SELECT table_name FROM information_schema.tables
WHERE table_schema = 'public' AND table_name LIKE '%referral%';

-- To check referral settings:
SELECT setting_key, setting_value, is_active FROM referral_settings WHERE is_active = TRUE;